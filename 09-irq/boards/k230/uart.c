...
void uart_init(void)
{
	...
	reg_write(IER, 0);
	/* Disable FIFO — fewer dropped bytes on bare-metal spam. */
	reg_write(FCR, 0);
...
void uart_puts(const char *s)
{
	while (*s)
		uart_putc(*s++);
	/* Wait shift register empty so next printk does not collide. */
	while ((reg_read(LSR) & LSR_TEMT) == 0)
		;
}
